...
for (i = 1 : numCats)
    ...
    if (numVecs < numFolds)
        display('ERROR! Each category must have at least numFolds samples.');
        return;
    end
end
...
if (any(sum(foldSizes, 2) ~= vecsPerCat))
    display('ERROR! The sum of fold sizes did not equal the number of category vectors.');
    return;
end
